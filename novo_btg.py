...
# controle1   = pd.read_excel(r'C:\Users\lauro.telles\Desktop\Relatorio ANBIMA-20231007T124445Z-001\ANBIMA\ANBIMA Dezembro\BTG\Controle de Contratos - Atualizado Janeiro de 2024.xlsx',1,skiprows=1)
# base_de_clientes = pd.read_excel(r'C:\Users\lauro.telles\Desktop\Relatorio ANBIMA-20231007T124445Z-001\ANBIMA\ANBIMA Dezembro\BTG\Base BTG.xlsx')
# rentabilidade = pd.read_excel(r'C:\Users\lauro.telles\Desktop\Relatorio ANBIMA-20231007T124445Z-001\ANBIMA\ANBIMA Dezembro\BTG\Rentabilidade.xlsx',skiprows=1)

# base_de_clientes = base_de_clientes[['Retiradas', 'Aportes', 'PL Total',
#                 'Fundos', 'Perfil do Cliente','Conta']]

# controle1 = controle1[['Conta', 'Status','Início da Gestão',  'Data distrato',
#                     'Carteira','Taxa de Gestão', 'Benchmark TX. Perf', 'TX. Perf.']]

# rentabilidade = rentabilidade[['Conta', 'Rentabilidade',]]

# controle1['Conta'] = controle1['Conta'].astype(str).apply(lambda x: '00'+x).str[:-2]
# controle1 = controle1.iloc[:-5,:]
# btg_final = pd.merge(controle1,rentabilidade,on='Conta',how='outer')
# btg_final = btg_final.merge(base_de_clientes,on='Conta',how='outer').reset_index().drop(columns='index').iloc[:-1,:]
# # btg_final['Início da Gestão'] = pd.to_datetime(btg_final['Início da Gestão']).dt.strftime("%d/%m/%Y")
# # btg_final['Data distrato'] = pd.to_datetime(btg_final['Data distrato']).dt.strftime("%d/%m/%Y")
# # btg_final['TX. Perf.'] = ''
...
# controle = pd.read_excel(r'C:\Users\lauro.telles\Desktop\Relatorio ANBIMA-20231007T124445Z-001\ANBIMA\ANBIMA Dezembro\GUIDE\Controle de Contratos - Atualizado Janeiro de 2024.xlsx',2,skiprows=1)
# patrimio_liquido = pd.read_excel(r'C:\Users\lauro.telles\Desktop\Relatorio ANBIMA-20231007T124445Z-001\ANBIMA\ANBIMA Dezembro\GUIDE\PL Dezembro.xlsx')
# fundos = pd.read_excel(r'C:\Users\lauro.telles\Desktop\Relatorio ANBIMA-20231007T124445Z-001\ANBIMA\ANBIMA Dezembro\GUIDE\PL Dezembro.xlsx')
# patrimonio_liquid_mes_anterior = pd.read_excel(r'C:\Users\lauro.telles\Desktop\Relatorio ANBIMA-20231007T124445Z-001\ANBIMA\ANBIMA Dezembro\GUIDE\pl_fundos_novembro.xlsx')
# aportes_e_resgates = pd.read_excel(r'C:\Users\lauro.telles\Desktop\Relatorio ANBIMA-20231007T124445Z-001\ANBIMA\ANBIMA Dezembro\GUIDE\Aportes e resgates dezembro.xlsx')
...
# guide_final = guide_final.drop(columns=['PL_MES_ANTERIOR','MERCADO']).rename(columns={'PL Total Atual':'PL Total',
#                                                                                       'Valor_em_Fundos':'Fundos',
#                                                                                       'Benchmark TX.  Perf.' :'Benchmark TX. Perf'
#                                                                                       })
...
# arquivo_final['Taxa de Gestão'] = (arquivo_final['Taxa de Gestão']*100).map('{:.6f}'.format)
# arquivo_final['Rentabilidade'] = (arquivo_final['Rentabilidade']).fillna(0).map('{:.7f}'.format)
# arquivo_final['Retiradas'] = arquivo_final['Retiradas'].abs()
# duas_casa_decimais = ['Aportes', 'Retiradas', 'PL Total',  'Fundos']
# for colunas in duas_casa_decimais:
#      arquivo_final[colunas] = arquivo_final[colunas].fillna(0).map('{:,.2f}'.format)
...
# status_das_contas = {'Ativo':'Ativa','Inativo':'Inativa','Pode Operar':'Ativa','Encerrado':'Encerrada'}
# arquivo_final['Status'] = arquivo_final['Status'].replace(status_das_contas)
# arquivo_final['Perfil do Cliente'] = arquivo_final['Perfil do Cliente'].replace(np.nan,'Desatualizado')
# arquivo_final['Cobranca da taxa de performance'] = np.where(arquivo_final['Utiliza benchmark.1']=='Sim','Sim','Não')
# arquivo_final['Tipo de cobranca da taxa de performance'] = np.where(arquivo_final['Utiliza benchmark.1']=='Sim','Específica','')
# arquivo_final['Taxa de Gestão'] = arquivo_final['Taxa de Gestão'].replace('0,000000','0,000001')


# ## adicionando conta Anbima

# cod_anbima_ultimo_form_enviado = pd.read_excel(r'C:\Users\lauro.telles\Desktop\Relatorio ANBIMA-20231007T124445Z-001\ANBIMA\Relatorio Abima SET2023\Retroativo Novembro\novembro_anbima.xlsx')
...
# arquivo_final['Início da Gestão'] = pd.to_datetime(arquivo_final['Início da Gestão'])
# arquivo_final = arquivo_final[arquivo_final['Início da Gestão']< pd.to_datetime('2024-01-01')]

# encontrando_contas_novas = (arquivo_final['Início da Gestão']>pd.to_datetime('2023-12-01'))&(arquivo_final['Status']=='Ativa')
# arquivo_final.loc[encontrando_contas_novas, 'Código ANBIMA'] = '0011223344'
# arquivo_final['Início da Gestão'] = pd.to_datetime(arquivo_final['Início da Gestão']).dt.strftime("%d/%m/%Y")
# removendo_encerradas = (arquivo_final['Status' ] == 'Encerrada')&(arquivo_final['Código ANBIMA'].isnull())
# removendo_inativas = (arquivo_final['Status' ] == 'Inativa')&(arquivo_final['Código ANBIMA'].isnull())
...
# ## ------> Juntando arquivos xp:


# xp_e_avulsas = pd.read_excel(r'C:\Users\lauro.telles\Desktop\Relatorio ANBIMA-20231007T124445Z-001\ANBIMA\Arquivos Base\Arquivo com contas antigas xp e outras.xlsx')
# limpar_colunas = ['Data distrato','Tipo de cobranca da taxa de performance','Descricao da taxa de performance','Benchmark TX. Perf',]
# for colunas in limpar_colunas:    
#     xp_e_avulsas[colunas] = xp_e_avulsas[colunas].replace(0,'')
...
# # trazer arquivo para excel e csv
# dia_hoje = datetime.now().strftime('%Y-%m-%d')


# #arquivo_final.to_csv(f'ANBIMA___{dia_hoje}.csv',sep ='|', encoding='latin-1',index=False)
# # arquivo_final.to_excel(f'EXCEL_Anbima{dia_hoje}.xlsx')


# #Leitura arquivo de retorno e correções


# comparativo = pd.read_csv(r'C:\Users\lauro.telles\Desktop\Relatorio ANBIMA-20231007T124445Z-001\PP4907512402001_comparativo_11.csv',sep ='|', encoding='latin-1')
